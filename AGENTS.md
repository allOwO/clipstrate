...
xcodebuild -scheme ChopClip build
xcodebuild -scheme ChopClip test
```
...
- UserDefaults 只经 `Shared/Settings.swift` 访问；日志用 `os.Logger(subsystem: "io.github.allowo.chopclip", category: 模块)`；关键路径打 `os_signpost`（点位见 02 §9）。
