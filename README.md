...
- **轻量**：常驻内存目标 <30MB。列表只加载预览文本，完整全文按需回源；图片缓存有硬上限并随系统内存压力回落。
...
- **Lightweight** — target resident memory <30 MB. Lists load preview text only and fetch full text on demand; image caches are hard‑capped and released under memory pressure.
