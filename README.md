...
- **轻量**：常驻内存实测约 33MB（空载，使用时略升但有上限）。列表只加载预览文本，完整全文按需回源；图片缓存有硬上限并随系统内存压力回落。
...
- **Lightweight** — ~33 MB resident in practice (idle; rises modestly under use but stays bounded). Lists load preview text only and fetch full text on demand; image caches are hard‑capped and released under memory pressure.
